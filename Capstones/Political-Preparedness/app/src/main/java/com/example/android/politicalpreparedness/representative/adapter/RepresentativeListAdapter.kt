...
class RepresentativeViewHolder(val binding: ListItemRepresentativeBinding): RecyclerView.ViewHolder(binding.root) {
    ...
    companion object {
        ...
        fun from(parent: ViewGroup): RepresentativeViewHolder {
            ...
            val binding = ListItemRepresentativeBinding.inflate(layoutInflator,parent, false)
    ...
    private fun showSocialLinks(channels: List<Channel>) {
        ...
        if (!facebookUrl.isNullOrBlank()) { enableLink(binding.representativeSocialFacebook, facebookUrl) }

        val twitterUrl = getTwitterUrl(channels)
        if (!twitterUrl.isNullOrBlank()) { enableLink(binding.representativeSocialTwitter, twitterUrl) }
    }

    private fun showWWWLinks(urls: List<String>) {
        enableLink(binding.representativeSocialWeb, urls.first())
    }
